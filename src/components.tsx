...
export function Modal({ title, onClose, children, className = "" }: { title: string; onClose: () => void; children: ReactNode; className?: string }) {
  ...
  return <dialog ref={ref} aria-label={title} className={className} onCancel={e => { e.preventDefault(); e.stopPropagation(); onClose(); }}><header className="modal-head"><h2>{title}</h2><button aria-label="Fermer" onClick={onClose}><X size={21} /></button></header>{children}</dialog>;
}
export function ModelPicker({ title, kind, names, value, onSelect, onClose, onTriggers }: { onTriggers?: (words: string[]) => void; title: string; kind: string; names: string[]; value: string; onSelect: (name: string) => void; onClose: () => void }) {
  ...
  return <Modal title={title} onClose={onClose} className="model-dialog"><label className="search-box"><Search size={19} /><input autoFocus aria-label="Rechercher un modèle" placeholder="Nom du modèle…" value={search} onChange={e => { setSearch(e.target.value); setLimit(60); }} /></label><div className="model-filter"><span className="muted">{filtered.length} disponibles</span><button aria-pressed={onlyFavorites} disabled={!ready} onClick={() => { setOnlyFavorites(v => !v); setLimit(60); }}><Heart size={16} fill={onlyFavorites ? "currentColor" : "none"} /> Favoris</button></div>{error && <p role="alert" className="hint">{error}</p>}<div className="model-list">{filtered.slice(0, limit).map(name => <div key={name} className={`model-card ${name === value ? "selected" : ""}`}><button className="model-choice" onClick={() => { onSelect(name); onClose(); }}><Picture path={modelPath(kind, name)} alt="" thumbnail /><span><strong>{shortName(name)}</strong><small>{name}</small></span>{name === value && <Check size={20} />}</button>{kind === "loras" && <button className="lora-detail-button" aria-label={`Fiche ${shortName(name)}`} onClick={() => setDetail(name)}>Fiche</button>}<button className="model-star" aria-label={`${favorites.includes(name) ? "Retirer des favoris" : "Mettre en favori"} ${shortName(name)}`} aria-pressed={favorites.includes(name)} disabled={!ready || saving} onClick={() => void favorite(name)}><Heart size={20} fill={favorites.includes(name) ? "currentColor" : "none"} /></button></div>)}</div>{filtered.length > limit && <button className="wide" onClick={() => setLimit(n => n + 60)}>Afficher la suite</button>}{!filtered.length && <p>{onlyFavorites ? "Marquez vos modèles avec le cœur pour les retrouver ici." : "Aucun modèle ne correspond à cette recherche."}</p>}{detail && <LoraDetails name={detail} onClose={() => setDetail(null)} onTriggers={onTriggers} onSelect={() => { onSelect(detail); onClose(); }} />}</Modal>;
}
