...
export default function Viewer({
  ...
  onClose: () => void;
  onFavorite: (item: ViewItem, favorite: boolean) => Promise<void>;
  onTrash: (item: ViewItem) => Promise<void>;
  onReuse: (item: ViewItem, url: string) => Promise<void>;
  onMore?: () => Promise<ViewItem[]>;
}) {
  const item = items[index],
    [urls, setUrls] = useState<Record<string, string>>({}),
    [message, setMessage] = useState(""),
    [busy, setBusy] = useState(false),
    [pose, setPose] = useState<Pose>(origin),
    [drag, setDrag] = useState<Point>({ x: 0, y: 0 });
  const stage = useRef<HTMLDivElement>(null),
    rail = useRef<HTMLDivElement>(null),
    image = useRef<HTMLImageElement>(null),
    poseRef = useRef(origin),
    points = useRef(new Map<number, Point>()),
    gesture = useRef<{
      start: Point;
      pose: Pose;
      pinch?: { center: Point; distance: number };
      axis?: "x" | "y";
      multiple: boolean;
    } | null>(null);
  const loaded = useRef(new Map<string, string>()),
    ...
    lock = useRef(false),
    alive = useRef(true),
    entering = useRef<Point | null>(null),
    previousTap = useRef(0);
  const handoff = useRef<Animation | null>(null),
    keep = useRef(new Set<string>());
  ...
  function clamp(next: Pose): Pose {
    ...
    const x = Math.max(
        0,
        (img.naturalWidth * ratio * next.scale - box.width) / 2,
      ),
      y = Math.max(
        0,
        (img.naturalHeight * ratio * next.scale - box.height) / 2,
      );
    return {
      scale: next.scale,
  ...
  async function fetchItem(current: ViewItem, required = false) {
    if (loaded.current.has(current.path))
      return loaded.current.get(current.path)!;
    let task = pending.current.get(current.path);
    if (!task) {
      task = (async () => {
        const value = current.url || (await imageUrl(current.path));
      ...
      if (alive.current && (required || keep.current.has(current.path))) {
        if (required) {
          ...
          for (const key of loaded.current.keys())
            if (key !== item.path && key !== current.path)
              loaded.current.delete(key);
        }
        const size = [...loaded.current.values()].reduce(
          (n, u) => n + u.length * 2,
          0,
        );
        if (
          ...
          size + value.length * 2 < 96 * 1024 ** 2
        )
          loaded.current.set(current.path, value);
        setUrls(Object.fromEntries(loaded.current));
      }
      return value;
    } finally {
      pending.current.delete(current.path);
    }
  }
  useEffect(() => {
    keep.current = new Set(
      items.slice(Math.max(0, index - 1), index + 2).map((i) => i.path),
    );
    for (const key of loaded.current.keys())
      if (!keep.current.has(key)) loaded.current.delete(key);
    let live = true;
    if (item)
      void fetchItem(item)
        .then(async (current) => {
          ...
          for (const neighbour of [items[index + 1], items[index - 1]])
            if (live && neighbour) await fetchItem(neighbour).catch(() => {});
        })
        .catch(() => {
          if (live)
            setMessage(
  ...
  useLayoutEffect(() => {
    ...
    const entry = entering.current;
    entering.current = null;
    if (entry && rail.current)
      rail.current.animate(
        [
          { transform: `translate(${entry.x}px, ${entry.y}px)` },
          { transform: "translate(0,0)" },
        ],
        { duration: duration(), easing: "cubic-bezier(.22,.68,.2,1)" },
      );
  }, [item?.path]);
  ...
  async function animateTo(target: Point) {
    const animation = rail.current?.animate(
      [
  ...
  const move = (delta: number) =>
    void run(async () => {
      ...
      if (next < 0 || (next >= items.length && !onMore)) {
        setDrag({ x: 0, y: 0 });
        return;
      }
      ...
      const animation = await animateTo({
        x: 0,
        y: -delta * (stage.current?.clientHeight ?? innerHeight),
      });
      // The rail already contains decoded neighbouring images, avoiding a blank frame.
      handoff.current = animation ?? null;
      onIndex(next);
    });
  const horizontal = (remove: boolean) =>
    void run(async () => {
      if (!item.gallery) return;
      if (remove) {
        const following = items[index + 1] ?? items[index - 1];
        if (following) await fetchItem(following, true);
        const animation = await animateTo({
          x: -(stage.current?.clientWidth ?? innerWidth),
          y: 0,
        });
        try {
          handoff.current = animation ?? null;
          entering.current = {
            x: 0,
            y: stage.current?.clientHeight ?? innerHeight,
          };
          await onTrash(item);
        } catch (e) {
          entering.current = null;
        ...
        const animation = await animateTo({ x: 0, y: 0 });
        animation?.cancel();
      }
    });
  useEffect(() => {
    const key = (e: KeyboardEvent) => {
      ...
      if (e.key.toLowerCase() === "f" && item?.gallery) {
        e.preventDefault();
        void run(() => onFavorite(item, !item.gallery?.favorite));
      }
      ...
      if (e.key === "+" || e.key === "=")
        setView(
          clamp({
            ...poseRef.current,
            scale: Math.min(5, poseRef.current.scale + 0.5),
          }),
        );
      if (e.key === "-")
        setView(
          clamp({
            ...poseRef.current,
            scale: Math.max(1, poseRef.current.scale - 0.5),
          }),
        );
    };
  ...
  return (
    <Modal title={tr("Votre image")} onClose={onClose} className="photo-viewer">
      <div
        ...
        ref={stage}
        data-scale={pose.scale.toFixed(2)}
        onDoubleClick={() => setView(origin)}
        onWheel={(e) => {
          if (e.ctrlKey || poseRef.current.scale > 1)
            setView(
              clamp({
                ...poseRef.current,
                scale: Math.min(
                  5,
                  Math.max(1, poseRef.current.scale - e.deltaY * 0.005),
                ),
              }),
            );
        }}
          ...
          if (values.length === 1)
            ...
            gesture.current = {
              ...
              pinch: {
                ...
                center: middle(values[0], values[1]),
              },
            };
          }
        }}
        onPointerMove={(e) => {
          ...
          if (values.length >= 2 && g.pinch) {
            const scale = Math.min(
                ...
                Math.max(
                  1,
                  (g.pose.scale * distance(values[0], values[1])) /
                    Math.max(1, g.pinch.distance),
                ),
              ),
              mid = middle(values[0], values[1]),
              box = e.currentTarget.getBoundingClientRect(),
              ratio = scale / g.pose.scale;
            setView(
              clamp({
                scale,
                x:
                  ...
                  box.left -
                  box.width / 2 -
                  (g.pinch.center.x - box.left - box.width / 2 - g.pose.x) *
                    ratio,
                y:
                  mid.y -
                  box.top -
                  box.height / 2 -
                  (g.pinch.center.y - box.top - box.height / 2 - g.pose.y) *
                    ratio,
              }),
            );
            return;
          }
          const x = e.clientX - g.start.x,
            y = e.clientY - g.start.y;
          if (g.pose.scale > 1) {
            setView(clamp({ ...g.pose, x: g.pose.x + x, y: g.pose.y + y }));
            return;
          }
          if (g.multiple) return;
          if (!g.axis && Math.hypot(x, y) > 12)
            g.axis = Math.abs(x) > Math.abs(y) * 1.2 ? "x" : "y";
          setDrag(
            g.axis === "y"
              ? {
                  x: 0,
                  y:
                    (index === 0 && y > 0) ||
                    (index === items.length - 1 && !onMore && y < 0)
                      ? y * 0.2
                      : y,
                }
              : { x: item.gallery ? x : x * 0.2, y: 0 },
          );
        }}
        onPointerCancel={(e) => {
        ...
        onPointerUp={(e) => {
          ...
          const x = e.clientX - g.start.x,
            y = e.clientY - g.start.y;
          if (!g.multiple && Math.hypot(x, y) < 8) {
            const now = performance.now();
            if (now - previousTap.current < 300) setView(origin);
            previousTap.current = now;
          }
          if (g.multiple || g.pose.scale > 1) {
            setDrag({ x: 0, y: 0 });
            return;
          }
          if (g.axis === "y" && Math.abs(y) > 70) move(y < 0 ? 1 : -1);
          else if (g.axis === "x" && Math.abs(x) > 100 && item.gallery)
            horizontal(x < 0);
          else {
            void animateTo({ x: 0, y: 0 }).then((a) => {
              a?.cancel();
              if (alive.current) setDrag({ x: 0, y: 0 });
            });
          }
        }}
      >
        ...
        >
          {[-1, 0, 1].map((offset) => {
            const neighbour = items[index + offset],
              src = neighbour && (neighbour.url || urls[neighbour.path]);
            return (
              <div
                key={neighbour?.path ?? `empty-${offset}`}
                className="viewer-slide"
                aria-hidden={offset !== 0}
                style={{
                  top: `${offset * 100}%`,
                  visibility:
                    pose.scale > 1 && offset !== 0 ? "hidden" : undefined,
                }}
              >
                {src ? (
                  <img
                    ref={offset === 0 ? image : undefined}
                    className={offset === 0 ? "full-image" : "adjacent-image"}
                    src={src}
                    alt={offset === 0 ? item.name : ""}
                    draggable={false}
                    style={
                      offset === 0
                        ? {
      ...
      <div
        ...
        hidden={!item.gallery || Math.abs(drag.x) < 12 || pose.scale > 1}
        ...
        style={{
          ...
          transform: `scale(${0.65 + Math.min(1, Math.abs(drag.x) / 100) * 0.35})`,
        }}
      >
        {drag.x < 0 ? (
          <Trash2 size={34} />
      ...
      <div className="viewer-actions">
        ...
        <button
          ...
          onClick={() =>
            void run(async () => {
              setMessage(
                await native<string>("save_image", {
                  dataUrl: url,
                  filename: item.name,
                }),
              );
