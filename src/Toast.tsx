...
export default function Toast({ message, error = false, onClose, children }: { message: string; error?: boolean; onClose: () => void; children?: ReactNode }) {
  ...
  return <div className={`toast ${error ? "toast-error" : ""}`} role={error ? "alert" : "status"}><span>{message}</span>{children}<button aria-label="Fermer la notification" onClick={onClose}><X size={18} /></button></div>;
}
