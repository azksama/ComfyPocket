...
export function parsePreset(input: unknown): InferencePreset {
  ...
  if (typeof p.name !== "string" || !p.name.trim() || p.name.length > 80 || !p.settings || typeof p.settings !== "object" || !["simple", "workflow"].includes(String(p.mode))) throw Error(tr("Le preset doit contenir un titre et des paramètres d’inférence."));
...
export function readPresets(): InferencePreset[] {
  ...
  if (!Array.isArray(value) || value.length > 100) throw Error(tr("Bibliothèque de presets invalide."));
  return value.map(parsePreset);
}
export function writePresets(presets: InferencePreset[]) {
  if (presets.length > 100) throw Error(tr("Vous pouvez mémoriser jusqu’à 100 presets."));
